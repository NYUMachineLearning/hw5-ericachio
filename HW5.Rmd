...
A regression tree was created for the boston housing data. The target value is medv which is  median value of owner-occupied homes in USD 1000’s.
Key varibles were rm ( average number of rooms per dwelling), lstat(percentage of lower status of the population), and dis (weighted distances to five Boston employment centres).
A regression tree paritions it into smaller groups. If we bag the tree, we can decrease its variance.  
```{r}
fit <- rpart(medv ~ .-medv, method="anova", data=boston)
...
tree.boston =  plot(fit, uniform=TRUE, main="regression tree for boston housing")
...
In bagging, we are decreasing the variance.
Our test error is generally close to the OOB, meaning that we have a generally good estimate to true test error. 
```{r}
#bagging

#The OOB is calculated from observations not part of the bootstrap models in an effort to calculate the miscalculation error. 
...
Boosting is when each tree is dependent on the tree before it, so that each iteration becomes closer and closer to fitting the data. Looking at our plot, we see that MSE (mean squared error) is lowest around 900/1000 trees before it levels out. 
```{r}
#boasting
...
# #Visualize important variables of interest
# plot(boost.carseats,i="lstat")
# plot(boost.carseats,i="rm")
